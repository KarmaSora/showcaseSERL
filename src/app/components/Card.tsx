...
function Card({
  ...
  return (
    <section className='card m-5 mx-auto h-[500px] w-[400px] overflow-hidden rounded-lg border border-gray-300 bg-white p-4 text-center shadow-lg transition-transform duration-300 hover:scale-105'>
      ...
      <Image
        ...
        width={400}
        height={150}
        className='mb-4 h-[150px] w-full object-contain'
      />
      ...
      <ul className='card-tags mb-4 flex flex-wrap justify-center'>
        {tags.map((tag, index) => (
          <li
            key={index}
            className='tag m-1 rounded-md bg-red-700 px-3 py-1 text-xs text-white'
          >
            {tag}
          </li>
        ))}
